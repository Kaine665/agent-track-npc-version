...
### PROB-021：Docker Compose 读取多个 API Key 时只读取第一个
...
在 `.env` 文件中配置了多个 OpenRouter API Key（用逗号分隔），但 Docker Compose 只读取了第一个 key，导致多 API Key 故障转移功能无法正常工作。

**问题影响**：

- 影响范围：Docker Compose 配置（docker-compose.yml）、环境变量读取
- 严重程度：中等（影响多 API Key 故障转移功能）

**原因分析**：

1. **Docker Compose 变量替换问题**：
   - `docker-compose.yml` 中使用 `${OPENROUTER_API_KEY:-}` 读取环境变量
   - 当 `.env` 文件中的值包含逗号时，Docker Compose 可能只读取第一个值
   - 环境变量值没有被引号包裹，导致解析错误

2. **环境变量格式问题**：
   - `.env` 文件格式正确：`OPENROUTER_API_KEY=key1,key2,key3`
   - 但 Docker Compose 在解析时可能将逗号后的内容截断

**解决方案**：

**在 docker-compose.yml 中给环境变量值添加引号**
...
1. **Docker Compose 行为**：Docker Compose 在处理包含特殊字符（如逗号）的环境变量值时，需要用引号包裹
2. **简单有效**：添加引号是最简单的解决方案
3. **不影响功能**：修复后多 API Key 故障转移功能正常工作

**是否需要更新文档**：

- [x] 是 - 需要更新哪个文档：docker-compose.yml（已更新）、DEPLOYMENT_TROUBLESHOOTING.md
- [ ] 否

**文档更新内容**：

- 在 `docker-compose.yml` 中添加引号包裹环境变量值
- 在 `DEPLOYMENT_TROUBLESHOOTING.md` 中添加此问题的排查步骤

**相关文件**：

- 问题文件：`docker-compose.yml`
- 修复文件：`docker-compose.yml`
- 文档文件：`DEPLOYMENT_TROUBLESHOOTING.md`

**备注**：

- 这是 Docker Compose 配置问题，不是代码问题
- 关键点：环境变量值包含特殊字符（逗号）时需要用引号包裹
- 修复后多 API Key 故障转移功能正常工作
- 建议：所有包含特殊字符的环境变量值都应该用引号包裹
...
| 2025-11-22 | 记录 PROB-021：Docker Compose 读取多个 API Key 时只读取第一个的问题 |
| 2025-11-22 | 记录 PROB-021：Docker Compose 读取多个 API Key 时只读取第一个的问题 |
